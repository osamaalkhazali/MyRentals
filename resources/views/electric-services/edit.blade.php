            ...
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    <div class="flex justify-between items-center mb-6">
                        <h2 class="text-2xl font-semibold">Edit Electric Service (تعديل خدمة الكهرباء)</h2>
                    ...
                    <form action="{{ route('electric-services.update', $electricService) }}" method="POST">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                ...
                                <select name="apartment_id" id="apartment_id" required
                                    ...
                                    @foreach ($apartments as $apartment)
                                        <option value="{{ $apartment->id }}"
                                            {{ old('apartment_id', $electricService->apartment_id) == $apartment->id ? 'selected' : '' }}>
                            ...
                            <div>
                                ...
                                <input type="text" name="registration_number" id="registration_number" required
                                    value="{{ old('registration_number', $electricService->registration_number) }}"
                                    class="w-full rounded-md border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                            </div>
                            ...
                            <div>
                                ...
                                <input type="text" name="meter_number" id="meter_number" required
                                    value="{{ old('meter_number', $electricService->meter_number) }}"
                                    class="w-full rounded-md border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                            </div>

                            <div class="flex items-center gap-3">
                                <input type="checkbox" name="is_active" id="is_active" value="1"
                                    {{ old('is_active', $electricService->is_active) ? 'checked' : '' }}
                        ...
                        <div class="mt-6 flex justify-end gap-4">
                            ...
                            <button type="submit"
                                ...
                                Update Electric Service (تحديث خدمة الكهرباء)
                            </button>
                        </div>
                    </form>
                </div>
