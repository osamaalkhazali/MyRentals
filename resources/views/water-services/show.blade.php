            ...
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    <div class="flex flex-col gap-2 md:flex-row md:items-center md:justify-between mb-6">
                        <div>
                            <p class="text-sm text-gray-500 dark:text-gray-400">Water Service (خدمة المياه)</p>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div class="rounded-lg border border-gray-200 dark:border-gray-700 p-4">
                            <p class="text-sm text-gray-500 dark:text-gray-400">Registration number (رقم التسجيل)</p>
                            <p class="text-lg font-semibold">{{ $waterService->registration_number }}</p>
                        </div>
                        <div class="rounded-lg border border-gray-200 dark:border-gray-700 p-4">
                            <p class="text-sm text-gray-500 dark:text-gray-400">Meter number (رقم العداد)</p>
                            <p class="text-lg font-semibold">{{ $waterService->meter_number }}</p>
                        </div>
                        <div class="rounded-lg border border-gray-200 dark:border-gray-700 p-4">
                            <p class="text-sm text-gray-500 dark:text-gray-400">Status (الحالة)</p>
                            <p>
                                <span
                                    ...
                                    {{ $waterService->is_active ? 'Active (نشط)' : 'Inactive (غير نشط)' }}
                                </span>
                            </p>
                        </div>
                        <div class="rounded-lg border border-gray-200 dark:border-gray-700 p-4">
                            <p class="text-sm text-gray-500 dark:text-gray-400">Last updated (آخر تحديث)</p>
                    ...
                    <div class="mt-10">
                        <div class="flex items-center justify-between mb-4">
                            <h3 class="text-xl font-semibold">Recent readings (آخر القراءات)</h3>
                            <a href="{{ route('water-readings.create', ['water_service_id' => $waterService->id, 'return_to_service' => 1]) }}"
                                class="text-sm font-medium text-blue-600 hover:text-blue-800">Log reading (تسجيل قراءة)</a>
                        </div>
                        ...
                        @if ($waterService->waterReadings->isEmpty())
                            <div
                                ...
                                No readings recorded yet. (لا توجد قراءات بعد)
                                <a href="{{ route('water-readings.create', ['water_service_id' => $waterService->id]) }}"
                                    class="text-blue-600 hover:text-blue-800">Add the first reading (أضف أول قراءة)</a>.
                            </div>
                        @else
                                            ...
                                            <tr>
                                                ...
                                                <td class="px-6 py-4 text-sm">
                                                    <span
                                                        ...
                                                        {{ $reading->is_paid ? 'Paid (مدفوع)' : 'Pending (قيد الانتظار)' }}
                                                    </span>
                                                </td>
                                                <td class="px-6 py-4 text-sm text-right">
                                                    <a href="{{ route('water-readings.edit', $reading) }}"
                                                        class="text-indigo-600 hover:text-indigo-900 dark:text-indigo-400">Edit (تعديل)</a>
                                                </td>
                                            </tr>
                                        @endforeach
                                    </tbody>
