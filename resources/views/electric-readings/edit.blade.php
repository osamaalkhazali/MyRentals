            ...
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    <div class="flex justify-between items-center mb-6">
                        <h2 class="text-2xl font-semibold">Edit Electric Reading</h2>
                    ...
                    <form action="{{ route('electric-readings.update', $electricReading) }}" method="POST"
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                <label for="electric_service_id" class="block text-sm font-medium mb-2">Electric Service (خدمة الكهرباء)
                                    *</label>
                                <select name="electric_service_id" id="electric_service_id" required
                                    ...
                                    @foreach ($electricServices as $service)
                                        <option value="{{ $service->id }}"
                                            {{ old('electric_service_id', $electricReading->electric_service_id) == $service->id ? 'selected' : '' }}>
                            ...
                            <div>
                                ...
                                <input type="date" name="reading_date" id="reading_date"
                                    value="{{ old('reading_date', $electricReading->reading_date) }}" required
                                    class="w-full rounded-md border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                            </div>

                            <div>
                                <label for="reading_value" class="block text-sm font-medium mb-2">Reading Value (kWh) (قيمة القراءة)
                                    *</label>
                                <input type="number" step="0.01" name="reading_value" id="reading_value" required
                                    value="{{ old('reading_value', $electricReading->reading_value) }}"
                                    class="w-full rounded-md border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                            </div>
                            ...
                            <div>
                                ...
                                <input type="number" step="0.01" name="cost" id="cost" required
                                    value="{{ old('cost', $electricReading->cost) }}"
                                    class="w-full rounded-md border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                            </div>

                            <div class="flex items-center gap-3">
                                <input type="checkbox" name="is_paid" id="is_paid" value="1"
                                    {{ old('is_paid', $electricReading->is_paid) ? 'checked' : '' }}
                            ...
                            <div class="md:col-span-2">
                                <label for="image_path" class="block text-sm font-medium mb-2">Meter Reading Photo (صورة قراءة العداد)
                                    {{ $electricReading->image_path ? '(Replace)' : '' }}</label>
